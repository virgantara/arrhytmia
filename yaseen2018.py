...
def run_yaseen(X_train, X_test, y_train, y_test):
    model = SVC(
        kernel='rbf',
        gamma='auto'
    )
    ...
    hasil = calc_performance_multiclass(y_test, y_pred)

    print(hasil)

def run_our(X_train, X_test, y_train, y_test):
    is_using_optimized = True
    model = get_optimized_model_mlp(is_using_optimized=is_using_optimized)
...
headers = ["atr"+str(i) for i in range(1,31)]
headers = np.append(headers,'class')
# print(headers)
data = pd.read_csv("yaseen_features.csv",names=headers)
...
print("Hasil Yaseen")
print("========================================================")
run_yaseen(X_train, X_test, y_train, y_test)
#
...
print("Hasil Our")
print("========================================================")
run_our(X_train, X_test, y_train, y_test)
